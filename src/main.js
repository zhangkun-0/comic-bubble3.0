...
const PRO5_LINE_HEAD_PUNCTUATION = new Set(['，', '。', ',', '.', '、']);
...
function pro5_fixLineHeadPunctuation(lines) {
  if (!Array.isArray(lines)) return [];
  if (lines.length === 0) return [];
  const result = [];
  lines.forEach((line, index) => {
    let current = typeof line === 'string' ? line : line == null ? '' : String(line);
    if (index === 0) {
      result.push(current);
      return;
    }
    let moved = '';
    while (current.length > 0 && PRO5_LINE_HEAD_PUNCTUATION.has(current[0])) {
      moved += current[0];
      current = current.slice(1);
    }
    if (moved && result.length) {
      const lastIndex = result.length - 1;
      result[lastIndex] = (result[lastIndex] || '') + moved;
    }
    result.push(current);
  });
  return result;
...
async function pro5_drawBubbleTextsOnCanvas(ctx) {
  ...
  for (const bubble of list) {
    ...
    let lines = pro5_wrapLines(ctx, text, innerW);
    lines = pro5_fixLineHeadPunctuation(lines);
    if (!lines.length) {
      ctx.restore();
      continue;
    }
...
function drawBubblesToContext(ctx, options = {}) {
  ...
  state.bubbles.forEach((bubble) => {
    ...
    if (includeText) {
      ...
      let lines = displayText ? displayText.split('\n') : [''];
      lines = pro5_fixLineHeadPunctuation(lines);
